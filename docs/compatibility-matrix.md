...
| GregTech CEu | 部分 | 注入容差（translucent 层 debug 标记放宽为 `require=0`，无模组类引用） | 2.8.10-beta（557242:5519022）：共存启动 `EntityRenderer` invalid classes 崩溃已修复——`GregTechTransformer` 用 ASM 把 `renderWorldPass` 第 4 处 `renderBlockLayer`（TRANSLUCENT）替换为 `BloomEffectUtil.renderBloomBlockLayer`，ordinal=3 注入 0 命中触发 require 校验失败；该处仅为 IrisGlDebug 阶段标记，已放宽；GTCEu Bloom 泛光画面表现未验证（参照 [docs/compat/lumenized.md](compat/lumenized.md) 同类限制），其对 RenderChunk/RegionRenderCacheBuilder 等的其余 ASM 改写未审计 |
