...
- `MixinBloomEffectUtilStateGuard`：在 `renderBloomBlockLayer` 入口/出口用
  ...
  活动纹理单元、0/1 号单元纹理绑定与 TEXTURE_2D 使能、混合/深度/剔除状态、program 0）。
  bloom 流程会重绑 FBO、跑外部 CCL shader program、把 0/1 号纹理单元指向自己的 FBO
  纹理，泄漏的状态会污染紧随其后的第一人称手部渲染（手部与所持物品全黑）。
...
- **第一人称手部模型与所持物品全黑**：bloom 流程 GL 状态泄漏污染手部渲染；
  由 `BloomStateGuard` 快照/恢复修复。GTCEu 与 Lumenized 症状相同（同一份 bloom 代码）。
...
- 2026-08-28：GTCEu 2.8.10-beta（curse 557242:5519022）+ CubicChunks 共存启动不再崩溃
  （实机确认）；随后发现手部/所持物品全黑，定位为 bloom 流程状态泄漏，
  本次提交新增 `BloomStateGuard` 修复（实机回归待确认，首次日志会输出漂移字段清单）。
