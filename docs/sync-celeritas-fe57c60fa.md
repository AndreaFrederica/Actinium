...
> 状态：**代码同步完成**。上游 `7190f87d8..fe57c60fa` 共 11 个提交：6 个收编（含光栅化
> 遮挡剔除三件套合并收编、1 个部分收编），5 个不适用。本地产出 7 个 commit。
...
- **遮挡剔除实机回归**（重点）：选项开/关对比——山洞/建筑内向外看区块渲染计数下降、
  无误剔除（闪块/地形空洞）；第三人称、行走 bobbing、开启光影包（MakeUp/BSL）各测一遍。
- **调度器三连实机回归**：大量区块更新（飞行/传送/世界加载）下构建吞吐无回退；
  注意 `REBUILD_LIST_FRAMES=2` 使首发建块列表深度从 10× 降为 2× in-flight target
  （上游有意为之），低帧率+大视距下目测首发加载。
- 上一轮 `sync-celeritas-7190f87d.md` 的遗留项（planar fog 观感、shadow pass、
  GUI 滑块、C 类运行专项）仍未执行，可与本轮合并起客户端验证。
