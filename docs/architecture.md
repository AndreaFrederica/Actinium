...
- 窗口准备（`SectionLattice.ensureWindowCovers`，可能触发 allocate/shiftRebase/rebase 就地
  改写数组）只能经 `SectionGraph.ensureWindowCovers` 在无搜索在飞时执行。两个 pass 的窗口
  准备统一在 `RenderSectionManager.updateForShadowPass` 提交任何搜索之前完成；阴影搜索的
  窗口中心最终以地形搜索的 root 为准（其 root 必须落在 lattice 内部）。
